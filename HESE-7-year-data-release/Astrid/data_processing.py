...
# filepath: \\wsl.localhost\Ubuntu\home\astridaurora\HESE-7-year-data-release\HESE-7-year-data-release\Astrid\data_processing.py

def setup_paths():
    """
    Set up system paths for external resources.
    """
    sys.path.insert(0, base_path + "/resources/external/")

def load_flux_data():
    """
    Load flux data from the file specified by FLUX_FILE.

    Returns:
        pd.DataFrame: DataFrame containing the flux data.
    """
    flx_df = pd.read_csv(FLUX_FILE)
    return flx_df
